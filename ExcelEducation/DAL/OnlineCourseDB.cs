...
INNER JOIN dbo.TBL_COURSE_MASTER ON dbo.TBL_COURSE.M_COURSE_ID = dbo.TBL_COURSE_MASTER.M_COURSE_ID where dbo.TBL_COURSE.ISACTIVE='Yes' and dbo.TBL_COURSE.PRICE IS NOT NULL order by dbo.TBL_COURSE.COURSE_ID Desc";
                ...
                // Dapper query to fetch the list of courses
        ...
        public async static Task<OnlineCourseModel> LoadCourseById(int id)
        {
            using (IDbConnection db = new SqlConnection(Connection.MyConnection()))
            {
                db.Open();
                ...
                var query = @"
                    SELECT 
                        ...
                        CAST(VIDEO_HR AS INT) AS DurationMonths,
                        TOTAL_NO_LECT AS NumberOfLectures,
                        '' AS ModeOfClasses,
                        OFFLINE_FEES AS CourseFees,
                        DISCOUNT AS DiscountedFees,
                        DISCOUNT_TITLE AS DiscountNote,
                        THUMBNAIL AS ImageUrl,
                        '' AS StartDateText,
                        PRODUCT_ORDER AS DisplayOrder
                    FROM TBL_COURSE
                    WHERE COURSE_ID = @ID";

                // Dapper query to fetch a single course by ID
                var res = await db.QueryAsync<OnlineCourseModel>(query, new { ID = id });
                return res.FirstOrDefault(); // Return the first match (or null if not found)
            }
        }
    }
}
